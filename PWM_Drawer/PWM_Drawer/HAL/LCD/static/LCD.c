...
void LCD_SelectRowColumn(uint8 Row, uint8 Column) {
	uint8 uint8Local_AddressInstruction = 0x00U;
	switch (Row) {
		case 0:
			uint8Local_AddressInstruction = (LCD_DDRAM_PREFIX | LCD_1ST_LINE_ADDRESS_PREFIX | Column);
			break;
		case 1:
			uint8Local_AddressInstruction = (LCD_DDRAM_PREFIX | LCD_2ND_LINE_ADDRESS_PREFIX | Column);
			break;
		case 2:
			uint8Local_AddressInstruction = (LCD_DDRAM_PREFIX | LCD_3RD_LINE_ADDRESS_PREFIX | Column);
			break;
		case 3:
			uint8Local_AddressInstruction = (LCD_DDRAM_PREFIX | LCD_4th_LINE_ADDRESS_PREFIX | Column);
			break;
		default: break;
	}
	LCD_SendInstruction(uint8Local_AddressInstruction);
}
...
void LCD_DisplayInteger(sint16 Data) {
	...
	uint8 uint8Local_Digit = 0U;
	char str_reversed[5] = "";
	char* str_ptr = str_reversed;
	do {
		uint8Local_Digit = Data%10;
		*str_ptr = (char)(48 + uint8Local_Digit); /* 48 is the ASCII code of '0' */
		Data = Data/10;
		str_ptr++;
	} while (Data > 0);
	do {
